...
namespace CCABN {
    ...
    WiFiCredentials WiFiManager::getCredentials() {
        ...
        credentials.hasPassword = (storedPassword.length() > 0);

        return credentials;
    }

    void WiFiManager::printCredentials() const {
        credentials.print();
    }

    void WiFiManager::saveCredentials(const WiFiCredentials& newCredentials) {
    ...
    bool WiFiManager::connectToWiFi(const WiFiCredentials &creds) {
        ...
        if (creds.hasPassword && creds.password.length() > 0) {
    ...
    bool WiFiManager::isConnected() const {
        return (WiFiClass::status() == WL_CONNECTED && status == WIFI_CONNECTED);
    }
    ...
    void WiFiManager::handleRoot() const {
        ...
        if (scanInProgress) {
            ...
            networksHtml = "<div class='no-networks'>No Networks</div>";
        }
    ...
    void WiFiManager::handleConnect() {
        ...
        newCreds.hasPassword = (password.length() > 0);
    ...
    void WiFiManager::handleNotFound() const {
        ...
        const String method = (server->method() == HTTP_GET) ? "GET" :
    ...
    String WiFiManager::scanNetworks() {
        ...
        if (numNetworks == 0) {
            ...
            for (int i = 0; i < numNetworks; i++) {
                ...
                const bool isSecured = (WiFi.encryptionType(i) != WIFI_AUTH_OPEN);
    ...
    void WiFiManager::handleScanStatus() const {
